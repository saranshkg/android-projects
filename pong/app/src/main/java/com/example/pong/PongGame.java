...
class PongGame extends SurfaceView implements Runnable {
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        switch(motionEvent.getAction() & MotionEvent.ACTION_MASK) {
            ...
            // This player lifted their finger from anywhere on screen.
    ...
    // When we start the thread with: mGameThread.start(); the run method is
    // continuously called by Android because we implemented the Runnable interface
    ...
    public void run() {
        ...
        while (mPlaying) {
            ...
            if (!mPaused) {
                update();
                //Now the bat and ball are in their new positions, we can see if there have been any collisions
                detectCollisions();
            }
    ...
    private void draw() {
        if (mOurHolder.getSurface().isValid()) {
            ...
            // unlockCanvasAnd Post is a method of SurfaceView
