...
class BulletHellGame extends SurfaceView implements Runnable{
    ...
    BulletHellGame(Context context, int x, int y) {// public??
        super(context);
    ...
    public void run() {
        while(mPlaying) {
            ...
            if(!mPaused) {
                update();
                // Now all the bullets have been moved and we can detect any collisions
                detectCollisions();
            }
        ...
        if(mOurHolder.getSurface().isValid()) {
            ...
            if(!mPaused) {
                mCanvas.drawText("Seconds Survived: " +
                        ...
                        mFontMargin, mScreenY - mFontMargin, mPaint);
            }
